...
def plot_energy_consumption(csv_file, start_line, graph_title):
    ...
    plt.plot(times, np.array(channel_1) / 68 * 1000 * 3.3)
    plt.xlabel('Time [s]')
...
def calculate_plateau_means(csv_file, start_line, plateauxlevels, delta):
    ...
    with open(csv_file, 'r') as file:
        ...
        for row in reader:
            times.append(float(row[1]))
            channel_1.append(3.3*float(row[2]) / 68 * 1000)
...
mean_values, time_extremes = calculate_plateau_means('energy_sequence_2.csv', 9, [9, 18.4], 0.25)
