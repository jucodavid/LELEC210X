...
class Chain:
    ...
    n_packets: int = 300  # Number of sent packets      HERE <----200 (pour le fun)
    ...
    def compensate_cfo_sto(self, y: np.array) -> np.array:
        """
        Compensates for CFO and STO in the received signal.

        :param y: The received signal, (N * R,).
        :return: The compensated signal, (N * R,).
        """
        if not self.bypass_cfo_estimation:
            cfo_est = self.cfo_estimation(y)
            t = np.arange(len(y)) / self.osr_rx / self.bit_rate
            y *= np.exp(-1j * 2 * np.pi * cfo_est * t)

        if not self.bypass_sto_estimation:
            sto_est = self.sto_estimation(y)
            y = np.roll(y, -sto_est)
    ...
    def modulate(self, bits: np.array) -> np.array:     #DPSK
        """
        Modulates a stream of bits using Differential Phase Shift Keying (DPSK).
        Modulation based on phase difference between consecutive symbols.

        :param bits: The bit stream, (N,).
        :return: The modulated bit sequence, (N * R,).
        """
        R = self.osr_tx  # Oversampling factor (transmitter)
        B = self.bit_rate  # Bit rate
        fd = self.freq_dev  # Frequency deviation (carrier frequency)
        
        # Create an empty array to store the modulated signal
        x = np.zeros(len(bits) * R, dtype=np.complex64)
        
        # Phase reference waveform
        ph = 2 * np.pi * fd * (np.arange(R) / R) / B  # Reference phase for symbol
        
        # Initial phase (start with phase 0)
        phase = 0

        for i, b in enumerate(bits):
            # If bit is 1, change phase by +pi (Differential Phase Shift Keying)
            if b == 1:
                phase += np.pi
            
            # Modulate the signal: generate the waveform with the current phase
            x[i * R : (i + 1) * R] = np.exp(1j * (phase + ph))  # Phase of the current symbol
            
        return x


    def demodulate(self, y: np.array) -> np.array:      #DPSK
        """
        Demodulates a stream of bits using Differential Phase Shift Keying (DPSK).
        Demodulation based on phase difference between consecutive symbols.

        :param y: The received modulated signal, (N * R,).
        :return: The demodulated bit stream, (N,).
        """
        R = self.osr_rx  # Oversampling factor (receiver)
        B = self.bit_rate  # Bit rate
        
        # Number of symbols (bits)
        nb_syms = len(y) // R
        bits_hat = np.zeros(nb_syms, dtype=int)
        
        # Time vector for one symbol (not needed for this function but can be used for clarity)
        T = 1 / B  # Symbol period
        t = np.arange(0, T, 1 / self.osr_rx)  # Sampling points per symbol
        
        # Initial phase of the first symbol
        prev_phase = np.angle(np.mean(y[:R]))  # Phase of the first symbol
        
        for i in range(1, nb_syms):
            # Extract the current symbol (R samples)
            symbol = y[i * R : (i + 1) * R]
            
            # Calculate the phase of the current symbol
            current_phase = np.angle(np.mean(symbol))
            
            # Calculate the phase difference between current and previous symbols
            phase_diff = current_phase - prev_phase
            
            # Normalize the phase difference to range [-pi, pi]
            phase_diff = (phase_diff + np.pi) % (2 * np.pi) - np.pi
            
            # If the phase difference is close to ±pi, bit = 1, else bit = 0
            bits_hat[i] = 1 if np.abs(phase_diff) > np.pi / 2 else 0
            
            # Update the previous phase for the next comparison
